...
RSpec.describe Tournament::TeamPolicy do

  let(:user) { FactoryBot.create(:user) }
  let(:team) { FactoryBot.create(:team_basis, owner: user) }
  let(:tournament_team) { FactoryBot.create(:tournament_team, team: team) }

  subject { described_class.new(user, tournament_team) }
